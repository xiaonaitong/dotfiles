...
(defun yari-anything (&optional rehash)
  ...
  (when current-prefix-arg (yari-ruby-obarray t))
  (anything 'yari-anything-source-ri-pages (yari-symbol-at-point)))

(defvar yari-anything-ri-pages
  `((name . "RI documentation")
    (init . (lambda ()
              (anything-init-candidates-in-buffer
               'local
...
(defun yari-anything ()
  (interactive)
  (anything :sources '(yari-anything-ri-pages)
        :buffer "*yari-anything*"
        :prompt "yari: "
...
;;; copied from helm.el, backport to anything
(defun anything-init-candidates-in-buffer (buffer data)
